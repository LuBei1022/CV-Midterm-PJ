...
7. 使用 SwanLab 对训练过程中的 loss、Precision、Recall 和 mAP 曲线进行可视化。
...
| `task2/relog_results_to_swanlab.py` | 将 YOLOv8 已有训练日志 `results.csv` 重新导入 SwanLab 进行可视化 |
...
训练过程中，YOLOv8 会自动保存训练日志文件 `results.csv`。该文件中记录了每个 epoch 的训练损失、验证损失、Precision、Recall、mAP@0.5 和 mAP@0.5:0.95 等指标。后续可以使用 `relog_results_to_swanlab.py` 将该日志重新导入 SwanLab，生成实验报告中需要的可视化曲线。

---

## 2.6 使用 SwanLab 可视化训练曲线

实验报告要求展示训练过程中在训练集和验证集上的 loss 曲线，以及验证集上的 Accuracy / mAP 曲线。由于本任务属于目标检测任务，因此主要使用 Precision、Recall、mAP@0.5 和 mAP@0.5:0.95 作为评价指标。

本项目提供了 `relog_results_to_swanlab.py`，用于读取 YOLOv8 训练后自动生成的 `results.csv` 文件，并将其中的训练指标重新导入 SwanLab 进行可视化。

该脚本不会重新训练模型，只会基于已有训练日志生成可视化曲线。

可视化内容包括：

1. 训练集上的 `train/box_loss`、`train/cls_loss`、`train/dfl_loss`；
2. 验证集上的 `val/box_loss`、`val/cls_loss`、`val/dfl_loss`；
3. 验证集上的 `metrics/precision(B)`、`metrics/recall(B)`；
4. 验证集上的 `metrics/mAP50(B)`、`metrics/mAP50-95(B)`。

### 2.6.1 登录 SwanLab

首次使用 SwanLab 前，需要先登录：

```bash
swanlab login
```

如果终端提示输入 API Key，可以在 SwanLab 网页端的个人设置中复制 API Key 并粘贴到终端。

---

### 2.6.2 修改 results.csv 路径

运行 `relog_results_to_swanlab.py` 前，需要确认脚本中的 `RUN_DIR` 指向自己电脑上 YOLOv8 训练结果所在目录，例如：
...
如果自己的训练结果路径不同，需要将 `RUN_DIR` 修改为实际路径。

---

### 2.6.3 运行 SwanLab 可视化脚本
...
```bash
python relog_results_to_swanlab.py
```

运行成功后，终端会输出 SwanLab 项目链接和 run 链接，例如：

```text
View project at https://swanlab.cn/...
View run at https://swanlab.cn/...
```

打开网页链接后，即可查看训练 loss、验证 loss、Precision、Recall 和 mAP 曲线。实验报告中的训练可视化截图即来自该页面。
...
由于数据集、模型权重和输出视频文件较大，本仓库默认不上传以下内容：
...
如果需要复现实验，请按照上述说明自行下载数据集、训练模型，并根据本地路径修改 `train_model.py`、`tracker.py` 和 `relog_results_to_swanlab.py` 中的相关路径。
# Task 3：图像分割任务
